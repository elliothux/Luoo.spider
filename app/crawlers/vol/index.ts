...
async function getVolInfo(volTask: VolTask): Promise<VolInfo> {
  ...
  const similarVols = R.map<Element, number>(
    (i: Element) =>
      getVolIdFromURL(i.querySelector(".cover-wrapper").getAttribute("href")),
...
async function getTrackInfoFromNode(
  ...
  const id = parseInt(
    trackNode.querySelector(".btn-action-like").getAttribute("data-id")
  );
  // 去除脏数据
  if (isNaN(id)) {
    return null;
  ...
  const artist = player
    ...
    .innerHTML.replace("Artist:", "")
    .trim();
  const album = player
    .querySelector("p.album")
    .innerHTML.replace("Album:", "")
    .trim();
  const cover = handleVolImgSrc(
    player
  ...
  // const color = await getAverageColor(cover);
  const color = "";
  const order = trackNode
    .querySelector(".trackname.btn-play")
    .innerHTML.slice(0, 2);
